...
public interface AssignmentRepository extends CrudRepository<Assignment, Long> {
    ...
    @Query(value = "SELECT * FROM Assignments WHERE DATE(deadline) >= CURDATE()", nativeQuery = true)
    public List<Assignment> getAllPendingAssignments();

    @Query(value = "SELECT * FROM Assignments WHERE course_code = ?3 AND DATE(deadline) >= ?1 AND DATE(deadline) <= ?2", nativeQuery = true)
